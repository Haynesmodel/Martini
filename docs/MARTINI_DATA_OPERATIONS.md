...
5. Only after explicit review, promote canonical assets and run `npm run generate:data`, `npm run test:assets`, and `npm run test:unit`. Generation emits a blocked manifest while both canonical sources are empty, or a promoted manifest after both sources contain complete, mapped seasons; partial promotion fails.
