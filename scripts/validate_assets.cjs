...
if (!games.length || !summaries.length) throw new Error('history assets must not be empty');
if (games.some(row => row.season > 2025 || row.teamA === row.teamB)) throw new Error('invalid history game');
if (summaries.some(row => row.season > 2025 || !/^franchise-\d+$/.test(row.owner))) throw new Error('invalid franchise summary');
