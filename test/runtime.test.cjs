...
test('runtime loader accepts blocked empty history with verified hashes', async () => {
  ...
  const fetchImpl = async url => { if (url.endsWith('asset-manifest.json')) return new Response(JSON.stringify(manifest), { status: 200 }); const body = files[Object.keys(files).find(file => url.endsWith(file))]; return new Response(body, { status: body === undefined ? 404 : 200 }); };
  const data = await runtime.loadHistory('/Martini/', fetchImpl); assert.equal(data.status, 'blocked'); assert.deepEqual(data.games, []); assert.deepEqual(data.summaries, []);
});
